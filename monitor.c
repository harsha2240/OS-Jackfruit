 ...
 * get_task_mm() / mmput() which can sleep. A spinlock would cause
 * a kernel BUG if held across a sleeping call.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
     ...
     * during iteration without use-after-free.
     */
    list_for_each_entry_safe(entry, tmp, &container_list, list) {
        rss = get_rss_bytes(entry->pid);

        /* Process no longer exists — remove the stale entry */
        if (rss < 0) {
            printk(KERN_INFO
                   "[container_monitor] PID %d (%s) gone, removing entry\n",
